...
import { SdmResponse, MitraLitabmas, AnggotaLitabmas, DokumenLitabmas, PenelitianResponse } from '../config/models';
...
export class syncToDB {
  ...
  static async syncPenelitian(): Promise<void> {
    try {
      ...
      for (let i = 0; i < daftarSdm.length; i++) {
        ...
        const penelitianList = await apiReader.fetchPenelitianByDosen(token, idSdm);
        ...
        for (const summary of penelitianList) {
          ...
          const p = await apiReader.fetchPenelitianDetail(token, summary.id);
          if (!p) continue;
