...
import { useProgramKosabangsa } from "../hooks/useProgramKosabangsa";
...
export function ProgramKosabangsa() {
  const { showForm, setShowForm, handleSubmit } = useProgramKosabangsa();
            ...
            <div className="space-y-6">
              {timeline.map((item, index) => (
                <div key={index} className="relative flex items-start gap-4 pl-10">
                  <div className={`absolute left-2.5 w-3 h-3 rounded-full border-2 ${item.status === "completed" ? "bg-green-500 border-green-500" :
                    item.status === "active" ? "bg-[#E30613] border-[#E30613] ring-4 ring-[#E30613]/20" :
                      "bg-white border-gray-300"
                    }`} />
                  <div className={`flex-1 bg-white border rounded-xl p-4 transition-shadow hover:shadow-md ${item.status === "active" ? "border-[#E30613]/30 shadow-sm" : "border-gray-100"
                    }`}>
