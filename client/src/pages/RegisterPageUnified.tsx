...
import { useRegisterUnified } from "../hooks/useRegisterUnified";
          ...
          {password.length > 0 ? (
            <div className="flex items-center gap-3 px-1">
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-200">
                <div
                  className={`h-full rounded-full transition-all ${passwordStrength === "weak"
                    ? "w-1/3 bg-rose-500"
              ...
              <span
                className={`text-xs font-semibold uppercase tracking-[0.18em] ${passwordStrength === "weak"
                  ? "text-rose-500"
                  : passwordStrength === "medium"
                    ? "text-amber-500"
                    : "text-emerald-600"
...
function FieldShell({ label, error, icon, trailing, children }: FieldShellProps) {
  return (
    <label className="block">
      ...
      <div
        className={`flex items-center gap-3 rounded-2xl border px-4 py-3 transition ${error
